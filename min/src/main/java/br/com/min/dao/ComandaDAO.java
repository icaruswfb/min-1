...
public class ComandaDAO extends BaseDAO{
	...
	public Double findTotalPagoPorCliente(Long clienteId){
		Session session = getSession();
		Query query = session.createSQLQuery("select sum(p.valor) from pagamento p join comanda c on p.comanda_id = c.id where p.formaPagamento != 'Cr�dito' and c.cliente_id = " + clienteId);
		Double result = (Double) query.uniqueResult();
		if(result == null){
			return 0d;
		}
