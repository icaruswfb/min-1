...
//		comanda = comandaService.persist(comanda, Utils.getUsuarioLogado(request).getPessoa());
	...
	private VerificacaoVO verificarComanda(Comanda comanda){
		...
		if(comanda.getValorCobrado() > comanda.getValorPago()){
			vo.addMessage(MessageVO.WARNING, "O valor cobrado � maior que o valor pago, fechar a comanda agora ir� gerar um d�bito.");
		}
		if(comanda.getValorCobrado() < comanda.getValorPago()){
			vo.addMessage(MessageVO.WARNING, "O valor cobrado � menor que o valor pago, fechar a comanda agora ir� gerar um cr�dito.");
		}
		for(LancamentoServico lancamentoServico : comanda.getServicos()){
			...
			if( ! hasServicoError && lancamentoServico.getServico() == null){
				...
				vo.addMessage(MessageVO.ERROR, "Todos os lan�amentos de servi�o devem ter um servi�o vinculado.");
			}
			if( ! hasFuncionarioError && lancamentoServico.getFuncionario() == null){
				hasFuncionarioError = true;
				vo.setCriticalError(true);
				vo.addMessage(MessageVO.ERROR, "Todos os lan�amentos de servi�o devem ter um funcion�rio vinculado.");
			}
			if( ! hasProdutoError ){
				for(LancamentoProduto lancamentoProduto : lancamentoServico.getProdutosUtilizados()){
					if(lancamentoProduto.getProduto() == null){
						...
						vo.setCriticalError(true);
						vo.addMessage(MessageVO.ERROR, "Todos os lan�amentos de produto do servi�o devem ter um produto vinculado.");
						break;
					}
				}
			}
		...
		for(LancamentoProduto lancamentoProduto : comanda.getProdutos()){
			...
			if( ! hasProdutoError && lancamentoProduto.getProduto() == null){
				hasProdutoError = true;
				vo.setCriticalError(true);
				vo.addMessage(MessageVO.ERROR, "Todos os lan�amentos de produto devem ter um produto vinculado.");
			}
			if(lancamentoProduto.getRevenda() == true){
				if( ! hasVendedorError && lancamentoProduto.getVendedor() == null){
					hasVendedorError = true;
					vo.setCriticalError(true);
					vo.addMessage(MessageVO.ERROR, "Todos os lan�amentos de produto de revenda devem ter um vendedor vinculado.");
				}
			}
		}
		...
		if(vo.getMessages().isEmpty()){
			vo.setSuccess(true);
			vo.addMessage(MessageVO.SUCCESS, "A comanda est� pronta para ser fechada.");
		}
	...
	public @ResponseBody Comanda fecharComanda(HttpServletRequest request){
		...
		throw new RuntimeException("Sem permiss�o para fechar a comanda");
	}
	...
	private Comanda criarComanda(HttpServletRequest request){
		...
		if(comanda.getFechamento() != null){
			throw new RuntimeException("Altera��o de comanda j� fechada");
		}
		return comanda;
		/*
		...
		if(diferencaTempo > 1000 || diferencaTempo < -1000){
			throw new RuntimeException("Comanda desatualizada. Atualize antes de fazer as altera��es.");
		}
