...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if(Agenda.esquerda == undefined){
				Agenda.esquerda = leftmost +                                  // leftmost possible
					(availWidth / (levelI + forward + 1) * levelI) // indentation
					* dis + (rtl ? availWidth - outerWidth : 0);   // rtl
				left = Agenda.esquerda;
			}else{
				left = Agenda.esquerda + (Agenda.largura * event.coluna);
			}
			seg.top = top;
			seg.left = left;
...
//				eventElement[0].style.width = Math.max(0, seg.outerWidth - seg.hsides) + 'px';
				if(Agenda.largura == undefined){
					Agenda.largura = Math.max(0, seg.outerWidth - seg.hsides) / 6;
				}
				eventElement[0].style.width = Agenda.largura + 'px';
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			" style=" +
				"'" +
				"position:absolute;" +
				"border-left: "+event.coluna+"px !important;" +
				"top:" + seg.top + "px;" +
				"left:" + seg.left + "px;" +
				skinCss +
				"'" +
